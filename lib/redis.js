...
exports.initialize = function initializeSchema(dataSource, callback) {
    ...
    if (dataSource.settings.password) dataSource.client.auth(dataSource.settings.password);
...
BridgeToRedis.prototype.save = function (model, data, opt, callback) {
    ...
    this.client.hgetall(model + ':' + data.id, function (err, prevData) {
        ...
        this.client.hmset([model + ':' + data.id, data], function (err) {
            if (err) return callback(err);
            // if opt includes ttl, add it
            if (opt && opt.ttl) {
                this.client.expire([model + ':' + data.id, parseInt(opt.ttl, 10)], function (err) {
                    if (err) return callback(err);
                    if (prevData) {
...
BridgeToRedis.prototype.create = function (model, data, opt, callback) {
    if (data.id) return create.call(this, data.id, opt, true);

    this.client.incr('id:' + model, function (err, id) {
        create.call(this, id);
    }.bind(this));

    function create(id, opt, upsert) {
        data.id = id.toString();
        this.save(model, data, opt, function (err) {
            if (callback) {
                callback(err, id);
            }
        });
